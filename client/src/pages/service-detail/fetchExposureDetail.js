    ...
    try {
      ...
      if (!svcs.length && !ings.length) {
        return {
          rows: [],
          emptyMessage:
            'No Service or Ingress found — deployment is not externally exposed.',
        }
      }
      ...
      for (const svc of svcs) {
        ...
        if (type === 'ClusterIP') {
          ...
          for (const p of ports) {
            rows.push([
              'Port',
              `${p.port}/${p.protocol || 'TCP'} → container ${p.targetPort}`,
            ])
          }
          ...
          for (const p of ports) {
            rows.push([
              'Port',
              `${p.port} → node port ${p.nodePort} → container ${p.targetPort}`,
            ])
          }
        } else if (type === 'LoadBalancer') {
          const ingress = svc.status?.loadBalancer?.ingress?.[0]
          const external =
            ingress?.ip || ingress?.hostname || 'Pending (no external IP yet)'
