...
| `FLAT_INGRESS_HOSTNAMES` | `false` | When `true`, joins the app name to `BASE_DOMAIN` with `-` instead of `.` (`appname-BASE_DOMAIN`), so the auto-filled hostname stays exactly one DNS label deeper than `BASE_DOMAIN`'s own parent zone — useful when a single-level wildcard TLS cert is scoped there and can't cover a nested subdomain. |
